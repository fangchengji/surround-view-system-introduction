...
void undistort_image(const cv::Mat& src, cv::Mat& dst, const cv::Mat& k, 
  const std::shared_ptr<DistortionProvider> dp) {
  // TODO: distort a single image
}

void undistortion_lut(const cv::Size& in_size, const cv::Size& out_size, float origin_pixel_size, 
